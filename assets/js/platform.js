(() => {
  ...
  if (!document.getElementById('mobile-enhancements')) {
    ...
    mobileLink.href = '/assets/css/mobile-enhancements.css?v=20260915a';
  ...
  API.applyTheme=(theme,persist=true)=>{theme=validTheme(theme)?theme:'dark';document.documentElement.dataset.theme=theme;const meta=document.querySelector('meta[name="theme-color"]');if(meta)meta.content=theme==='light'?'#dce8f5':'#010204';if(persist){localStorage.setItem(THEME_KEY,theme);localStorage.setItem(LEGACY_THEME,theme)}document.querySelectorAll('[data-platform-theme]').forEach(b=>{b.innerHTML=theme==='light'?ICONS.moon:ICONS.sun;b.setAttribute('aria-label',theme==='light'?'Dark mode':'Light mode')});window.dispatchEvent(new CustomEvent('tools:themechange',{detail:{theme}}));return theme};
